    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            for (int i = 1; i <= totalPassengers; i++) {
                ...
                boolean passeger = pd.createPassenger(pName, pPhoneNumber, pDob, pSex, id, newBookId.getBookingID());


                Passengers newPassenger = pd.getLatestPassengerByBookingId(newBookId.getBookingID()); // Cần tạo hàm này trong PassengersDAO
                ...
                if (newPassenger != null) {
                    if (flightDetailId2Str != null) {
                        ...
                        if (i >= adultTicket + 1 && i <= adultTicket + childTicket) {
                            td.createTicket(null, flightDetailId, seatId1, newPassenger.getPassengerID(), newBookId.getBookingID(), 1, totalPrice);
                            td.createTicket(null, flightDetailId2, seatId2, newPassenger.getPassengerID(), newBookId.getBookingID(), 1, totalPrice);
                            ...
                            td.createTicket(null, flightDetailId, seatId1, newPassenger.getPassengerID(), newBookId.getBookingID(), (pBaggages != null ? pBaggages : 0), totalPrice);
                            td.createTicket(null, flightDetailId2, seatId2, newPassenger.getPassengerID(), newBookId.getBookingID(), (pBaggages != null ? pBaggages : 0), totalPrice);
                        }
                    } else {
                        if (i >= adultTicket + 1 && i <= adultTicket + childTicket) {
                            td.createTicket(null, flightDetailId, seatId1, newPassenger.getPassengerID(), newBookId.getBookingID(), 1, totalPrice);
