                        ...
                        <div class="table-responsive shadow rounded">
                            <table class="table table-bordered bg-white mb-0 text-center align-middle">
                                ...
                                <tr>
                                    ...
                                    <th class="border-bottom p-1">Full name</th>
                                    <th class="border-bottom p-1">Date of birth</th>
                                    <th class="border-bottom p-1">Email</th>
                                    <th class="border-bottom p-1">Phone</th>
                                    <th class="border-bottom p-1">Role</th>
                                ...
                                <c:choose>
                                    <c:when test="${not empty accountList}">
                                        <c:forEach var="acl" items="${accountList}">
                                            <tr>
                                                <td class="p-1">${acl.accountId}</td>
                                                <td class="p-1">${acl.fullName}</td>
                                                <td class="p-1">${acl.dob}</td>
                                                <td class="p-1">${acl.email}</td>
                                                <td class="p-1">${acl.phone}</td>
                                                <td class="p-1">
                                                    <c:forEach var="role" items="${rolesList}">
                                                        <c:if test="${role.roleId == acl.roleId}">
                                                            ${role.roleName}
                                                        </c:if>
                                                    </c:forEach>
                                                </td>
                                                ...
                                                <td class=" p-1">
                                                    <a href="${pageContext.request.contextPath}/manageAccount?action=viewDetail&id=${acl.accountId}" class="btn btn-icon btn-sm btn-soft-primary"><i
                                                            class="uil uil-eye"></i></a>
                                                    <a href="${pageContext.request.contextPath}/manageAccount?action=update&id=${acl.accountId}" class="btn btn-icon btn-sm btn-soft-success"><i
                                                    ...
                                                    <c:choose>
                                                        <c:when test="${acl.status == 0}">
                                                            ...
                                                            <a href="javascript:void(0);"
                                                               onclick="confirmRestore(${acl.accountId})"
                                                        ...
                                                        <c:otherwise>
                                                            <!-- Nút xóa (vô hiệu hóa) -->
                                                            <a href="javascript:void(0);"
                                                               onclick="confirmDelete(${acl.accountId})"
                ...
                <div class="d-flex justify-content-center mt-3">
                    <div class="pagination">
                        <c:if test="${currentPage > 1}">
                            <a href="${pageContext.request.contextPath}/searchAccount?page=1&search=${search}&status=${status}&roleId=${roleId}"
